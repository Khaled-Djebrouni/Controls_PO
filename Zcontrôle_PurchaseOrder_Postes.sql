...
/****** Object:  View [dbo].[Zcontrôle_PurchaseOrder_Postes]    Script Date: 05.02.2024 8:45:19 ******/
SET ANSI_NULLS ON
GO
...
--sdbRollout_MM 
--sdbPILOT3_MM
CREATE VIEW [dbo].[Zcontrôle_PurchaseOrder_Postes] AS 
SELECT
    ...
    CASE
        WHEN PSTYP IS NULL AND KNTTP IS NULL THEN
            CASE
                WHEN PO.MATNR IS NULL THEN 'NOK, Code Article Stk Obligatoire'
                WHEN NOT EXISTS (SELECT 1 FROM sdbBackup_Material.dbo.BACKUP_External_Number B
                                 WHERE B.ANCIEN_CODE = concat(PO.WERKS,'_',PO.MATNR)
                                 AND B.MTART = 'ZAPP') THEN 'NOK, Article Stocké inexistant'
                ELSE 'OK'
            END
        WHEN	(PSTYP IS NOT NULL OR KNTTP IS NOT NULL ) AND 
				EXISTS (SELECT 1 FROM sdbBackup_Material.dbo.BACKUP_External_Number B
						WHERE B.ANCIEN_CODE = concat(PO.WERKS,'_',PO.MATNR)
						 AND B.MTART = 'ZAPP') THEN 'NOK, article stocké imputation doit etre vide '
        WHEN PSTYP IS NULL AND KNTTP = 'K' THEN
            CASE
                WHEN PO.MATNR IS NULL THEN 'NOK, Renseigner Code Article Non stocké'
                WHEN NOT EXISTS (SELECT 1 FROM sdbBackup_Material.dbo.BACKUP_External_Number B
                                 WHERE B.ANCIEN_CODE = PO.MATNR
                                 AND B.MTART = 'ZNST') THEN 'NOK, article Non Stocké Inexistant'
                ELSE 'OK'
            END
        WHEN KNTTP = 'A' AND PO.MATNR IS NOT NULL THEN 'NOK, Fiche Asset'
        ELSE 'OK'
    ...
    CASE
        WHEN (TXZ01 IS NULL OR TXZ01 = '') AND (PO.MATNR IS NULL OR PO.MATNR = '') THEN 'NOK, Obligatoire'
        WHEN (PO.MATNR IS NULL OR PO.MATNR = '') AND LEN(TXZ01) > 40 THEN 'NOK, Longueur'
        ELSE 'OK'
    ...
    PO.MEINS, -- Unité de mesure de la quantité commandée
    CASE
        WHEN	PO.MATNR IS NULL AND 
				PO.MEINS NOT IN (SELECT MSEH3 from dgSAP_SP1_100.dbo.T006A where SPRAS='F')
				THEN 'NOK, valeur hors liste'
		WHEN PO.MATNR IS NOT NULL AND 
			 PSTYP IS NULL AND 
			 PO.MEINS NOT IN (SELECT MSEH3 from dgSAP_SP1_100.dbo.T006A where SPRAS='F')
			 THEN 'NOK,  valeur hors liste'
		...
		WHEN PSTYP='P' and PO.MEINS<>'UO' THEN 'NOK, MEINS = UO'
        ELSE 'OK'
	...
	PO.MATKL, -- Groupe de matériel (peut être NULL)
    CASE
       WHEN (PO.MATNR IS NULL OR PO.MATNR = '') AND PO.MATKL IS NOT NULL AND PO.MATKL NOT IN ( select MATKL from dgSAP_SP1_100.dbo.T023 ) THEN 'NOK, valeur hors liste'
        WHEN LEN(PO.MATKL) > 9 THEN 'NOK, Longueur'
        ELSE 'OK'
...
LEFT JOIN	sdbBackup_Material.dbo.BACKUP_External_Number NumTab
			ON (	ANCIEN_CODE=concat(PO.WERKS,'_',PO.MATNR)	OR	ANCIEN_CODE=PO.MATNR)
